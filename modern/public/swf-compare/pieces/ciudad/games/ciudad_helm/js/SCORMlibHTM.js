...
function SCOInitialize() { 
	...
	if (!g_bInitDone) { 
		...
		// The SCOInitData function can be defined in another script of the SCO SCOInitData() 
	} 
...
function SCOFinish() { 
	if ((APIOK()) && (g_bFinishDone == false)) { 
		//if (typeof(SCOSaveData) != "undefined"){ 
			...
			// The SCOSaveData function can be defined in another script of the SCO SCOSaveData(); 
		//} 
...
function salirCurso() {
	if (APIOK()) {  
		parent.location.replace('../../delfos/cursos_est.faces');
	}
}

function glosarioDelfos() {
	if (APIOK()) {
		var fbar = window.parent.barra.document.forms['arbolScos:frmNavegar'];
		var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verGlosarioCurso.faces?cuCurso="+cursoId;
		
		//alert("strUrl:" + strUrl)
		
		var props = "fullscreen=no,height=550,width=750,status=no,toolbar=no,menubar=no,scrollbars=yes,location=no,top=50,left=50,resizable=yes";
		var popup = window.open(strUrl, "Glosario", props, true);
		popup.focus();
	}
}
...
function faqsDelfos() {
	if (APIOK()) {
		...
		var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verFaqsCurso.faces?cuCurso="+cursoId;
		
		//alert("strUrl:" + strUrl)
		
		var props = "fullscreen=no,height=550,width=750,status=no,toolbar=no,menubar=no,scrollbars=yes,location=no,top=50,left=50,resizable=yes";
		var popup = window.open(strUrl, "Faqs", props, true);
		popup.focus();
	}
}

function carpetaDelfos() {
	if (APIOK()) {
		var fbar = window.parent.barra.document.forms['arbolScos:frmNavegar'];
		var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verCarpetaCurso.faces?cursoId="+cursoId;
...
function tareaDelfos() {
	if (APIOK()) {
		var fbar = window.parent.barra.document.forms['arbolScos:frmNavegar'];
		var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verTareasCurso.faces?cuCurso="+cursoId;
		
		//alert("strUrl:" + strUrl)
		
		var props = "fullscreen=no,height=550,width=750,status=no,toolbar=no,menubar=no,scrollbars=yes,location=no,top=50,left=50";
		var popup = window.open(strUrl, "Faqs", props, true);
		popup.focus();
	}
}

function urlExterna(urlExt) {
	//alert("strUrl:" + strUrl);
	var props = "fullscreen=no,height=550,width=750,status=no,toolbar=no,menubar=no,scrollbars=yes,location=no,top=50,left=50,resizable=yes";
	var popup = window.open(urlExt, "Documento", props, true);
	popup.focus();
}

function abrirDocumento(documento) {	
	var temp = location.href;
	var curso = temp.substring(temp.indexOf("curso-"), temp.indexOf("1_index.htm") - 1);
	documento = documento.substring(documento.indexOf("/") + 1, documento.length);
	var props = "fullscreen=no,height=550,width=750,status=no,toolbar=no,menubar=no,scrollbars=yes,location=no,top=50,left=50,resizable=yes";
	var path = "download.jsp?curso=" + curso + "&archivo=" + documento;
	popup = window.open(path, "Documento", props, true);
...
function abrirForo() {
    if (APIOK()) {
		...
		//var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verForosCurso.faces";
...
function abrirChat() {
    if (APIOK()) {
		...
		//var cursoId = fbar.elements['arbolScos:frmNavegar:cuCurso'].value;
		var urlApp = getUrlDelfos();
		var strUrl = urlApp + "/verChatsCurso.faces";
...
function salirForo() {
    if (APIOK()) {
        var fModo = parent.parent.barra.document.forms['frmModo'];
        viewMode = fModo.elements['viewMode'].value
		var urlApp = getUrlDelfos();
        if (viewMode=="1") {
            parent.opener.location.replace(urlApp+"/verForosCom.faces");   
            parent.close();
        }
        else {
            parent.location.replace(urlApp+"/verForosCom.faces");
        }
    }
}

function getUrlDelfos() {
	var urlOrigen = "" + window.location + "";
	var posFinHost = urlOrigen.indexOf('/cursoscat');	
	var urlHost = urlOrigen.substring(0,posFinHost);	
	urlHost += "/delfos";
	return urlHost;
}
