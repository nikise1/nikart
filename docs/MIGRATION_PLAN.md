...
- Temporary Ruffle vs AwayFL compare kit: one HTML page per SWF with generic players (Ruffle autoplay on, unmute overlay hidden); menu/popWin open `/swf-compare/` instead of the popup. Movies and sidecars load from the committed copies at `/swf-compare/pieces/{id}/` (the `/static` origin proxy did not play on the Vercel preview). Compare stages use the SWF aspect-ratio so AwayFL’s absolutely positioned canvas cannot collapse the pane; `w`/`h` is the pane box (SHOW_ALL), not native SWF pixels or `100%` of the window.
