...
- Temporary Ruffle vs AwayFL compare kit: one HTML page per SWF with generic players (Ruffle autoplay on, unmute overlay hidden); menu/popWin open `/swf-compare/` instead of the popup. Movies and sidecars load through the `/static` rewrite to `static.nikart.co.uk` (same proxy as `/fl/away`), so child Loader URLs use the origin movie directory. Compare stages use the SWF aspect-ratio so AwayFL’s absolutely positioned canvas cannot collapse the pane; `w`/`h` is the pane box (SHOW_ALL), not native SWF pixels or `100%` of the window.
