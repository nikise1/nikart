...
class com.greensock.plugins.TweenPlugin {
		/** @private **/
		public static var VERSION:Number = 1.4;
		/** @private **/
		...
		private function addTween(object:Object, propName:String, start:Number, end:Object, overwriteProp:String):Void {
			if (end != undefined) {
				...
				if (change != 0) { //don't tween values that aren't changing! It's a waste of CPU cycles
					_tweens[_tweens.length] = new PropTween(object, propName, start, change, overwriteProp || propName);
				}
			}
		}
		...
		public function killProps(lookup:Object):Void {
			...
			while (--i > -1) {
				if (lookup[this.overwriteProps[i]]) {
		...
		public static function activate(plugins:Array):Boolean {
			...
			var i:Number = plugins.length, instance:Object;
			while (i--) {
				if (plugins[i].API == 1) {
